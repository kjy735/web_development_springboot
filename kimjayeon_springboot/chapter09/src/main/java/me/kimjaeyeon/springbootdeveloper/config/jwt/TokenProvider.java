package me.kimjaeyeon.springbootdeveloper.config.jwt;
...
/*
    1. makeToken():
        ...
        claim에는 유저 id 지정
        토큰을 만들 때 properties 파일에 선언해둔 비밀 값과 함께 HS3256 방식으로 암호화
    ...
    3. getAuthentication():
        토큰을 받아 인증 정보를 담은 객체 Authenticaion을 반환하는 메서드
        프로퍼티즈 파일에 저장한 비밀값으로 토큰을 복화한 뒤 클레임을 가져오는 private 메서드
        getClaims()를 호출, 클레임 정보를 반환받아 사용자 이메일이 들어이있는 토큰 제목
        sub와 토큰 기반으로 인증 정보를 생성
        이때, UserPasswordAuthenticaionToken의 첫 argument로 들어가는 User는
    ...
    4. getUserId():
        ...
        다음단계 -> 코드가 제대로ㄷ 동작하는지 확인하기 위해서 테스트 코드 작성 예정
        20250116에 하겠습니다.
*/
