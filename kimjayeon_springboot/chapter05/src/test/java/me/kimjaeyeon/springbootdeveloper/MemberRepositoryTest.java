...
class MemberRepositoryTest {
    ...
    @Sql("/insert-members.sql") // / : 상위로 올라가서 찾는다(경로)
    @Test
    void getAllMembers() {
        // when
        List<Member> members = memberRepository.findAll();  // member를 찾을때 find를 씀
        // memberRepository는 상속받은 메서드명을 그대로 가져가니까
        // 우리가 직접 정의하는 영어가 아니라도 익숙해져야 합니다.

        // then
        assertThat(members.size()).isEqualTo(3);    // list의 크기가 3이라면 통과할 것이다.
    }
    /*
        이상의 코드는 멤버 전체를 찾고, 그 리스트의 사이즈가 3인지를 확인하는 테스트

        이하의 코드는 id가 2인 멤보를 찾아보도록 합니다.
        ex)
        SELECT * FROM member WHERE id = 2;
        member라는 테이블의 모든 컬럼을 조회하는데, id가 2인 객체의 값들을 보여달라.
        FROM member,           *(asterisk : all), WHERE id = 2,    SELECT
     */
    @Sql("/insert-members.sql")
    @Test
    void getMemberId() {    // 하나를 찾아서 member에 s안붙임
        // when
    ...
    /*
        이제는 id가 아니라 name을 기준으로 찾기로 해보겠습니다.
        id = 기본키
    ...
    @Sql("/insert-members.sql")
    @Test
    void getMemberByName() {    // MemberRepository.java에서 findByName()이었다는 점에 주목
        // when
    ...
    @Test   // @Sql 애너테이션을 달지 않습니다. 이 메서드는 1, 'A'에 해당하는 객체를 저장할거라서
    void saveMember() {
        //given
    ...
    /*
        ...
        만약 여러 엔티티를 한꺼번에 저장하고 싶다면 savaAll() 메서드를 사용 가능
     */

    @Test
    void saveMembers() {
        //given - 다수의 객체를 저장할 예정이므로 collections를 사용할 겁니다 -> List
        List<Member> members = List.of(new Member(2L, "김재연"),
                new Member(3L, "김재연"));
