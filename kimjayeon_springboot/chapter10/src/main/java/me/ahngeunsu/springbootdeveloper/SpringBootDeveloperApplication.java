...
/*
    ...
    쿠키 관리 클래스를 구현할겁니다.
        ...
        현재 access_token을 기준으로 합니다 -> 저희는 지금 oauth를 통해서 로그인을 했습니다
        access_token과 refresh_token을 받았습니다.

        -> 로그인 이전에 이미 data.sql을 통해서 1-3번글까지는 있는 상황 각각 user1 - user3

        즉. 현재 우리가 로그인한 상태로는 새로운 근 생성 및 새로 생성된 글을 바탕으로 한
        수정 및 삭제가 가능합니다.
 */