...
/*
    New project 생성시 주의점:
        ...
        2. DLS -> Groovy 설정
        3. name = ArtifactID
        4. build.gradle 설정을 복사하는데, 복사 후에 -> sync를 꼭꼭 눌러주셔야 합니다.
            -> SpringBootDeveloperApplication에서 @SpringBootApplication 애너테이션에 빨간줄 가있다면
                새로고침 안했을 확률이 거의 98%
        5. resources 내에 static 내의 index.html이라고 하는데, 해당 폴더명의 경우 대부분의 개발자들이 합의한 형태
...
/*
    ...
    2. 스프링 컨셉
       ...
       2) 의존성 주입(DI)
        ...
        3) 빈과 스프링 컨테이너
            3)-1. 스프링 컨테이너 : 빈이 생성되고 소멸되기 가지의 생명주기를 관리함.
            ...
            3)-2. 빈 : 스프링 컨테이너가 생성하고 관리하는 객체 -> 이상의 코드들에서 B가 빈에 해당
                ...
                public class MyBean{}
                    ...
                    이상과 같은 @Component라는 애너테이션을 붙이면 MyBean 클래스가 빈으로 등록됨
                    이후 스프링 컨체이너에서 이 클래스를 관리하게 되고. 빈의 이름은 첫 문자를
                    소문자로 바꾸어서 관리함. 즉 틀래스 MyBean의 빈 이름은 myBean이 됨.
        ...
        4) 관점 지향 프로그래밍(AOP)
            Aspect-oriented Programming : 프로그래밍에 대한 관심을 핵심 관점 / 부가 관점
                ...
                ex) 계좌 이체, 고객 관리하는 프로그램이 있다고 가정할 때, 각 프로그램에는
                    로딩 조직, 즉 지금까지 일어난 일을 기록하기 위한 로직과 여러 데어터를 관리
                    하기 위한 데이터베이스 연결 로직이 포함됨. 이 때,
                        핵심 관점 - 계좌 이체, 고객 관리 룆ㄱ
        ...
        5) 이식 가능한 서비스 추상화(PSA)
            Portable Service Abstraction : 스프링에서 제공하는 다양한 기술들을 '추상화' 해
                ...
                ex) 클라이언트의 매핑 / 클래스, 매소드의 매핑을 위한 애너테이션들
    ...
    3. 스프링 부트 3 둘러보기
        첫 번째 스프링 부트 3 예제 만들기
            01 단계 -
                ...
                TestController.java파일을 만들겁니다.

       스프링 부트 스타터 살펴보기

            스프링 쿠트 스타터는 의존성이 모여있는 그룹에 해당함.
            ...
            spring-boot-stater-actuator : 모니터링을 위해 애플리케이션에서 제공하는 다양한 정보를
                제공하기 쉽게 하는 의존성 모음
            spring-boot-stater-jpa : ORM(추후 수업 예정)을 사용하기 위한 인터페이스의 모음인
