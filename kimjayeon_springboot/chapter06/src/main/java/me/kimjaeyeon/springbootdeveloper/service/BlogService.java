...
import me.kimjaeyeon.springbootdeveloper.repository.Blogrepository;
import org.springframework.stereotype.Service;

@RequiredArgsConstructor        // final이 붙거나 @NotNull이 붙은 필드의 생성자 추가
@Service    // 빈으로 등록 // 서비스 역할을 하는 애다 라는걸 명시해줌
public class BlogService {
    ...
    /*
        ...
        save() : JpaRepository에서 지원하는 저장 메서드 save()로,
            AddArticleRequest 클래스에 저장된 값들을 article(테이블명) 데이터베이스에 저장합니다.
        ...
        -> 컨트롤러 메서드 코드 작성하기
            ...
            이름에서 유추할 수 있듯 각 에너테이션들은 HTTP 메서드와 대응합니다.

            /api/articles에 POST 요청이 들어오면 @PostMapping을 이용해 요청을 매핑한 후,
            블로그 글을 생성하는 BlogService의 save() 메서드를 호출
            -> 생성된 블로그 글을 반환하는 작업을 하는 addArticle() 메서드를 작성할 예정.

            지시사항
            service와 동일한 라인에 controller 패키지 생성한 뒤에, BlogApiController.java생성
     */
}
