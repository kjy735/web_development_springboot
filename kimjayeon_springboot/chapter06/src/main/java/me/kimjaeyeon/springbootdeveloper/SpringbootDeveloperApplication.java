    ...
    1. 데이터베이스(database)란?
        ...
        DBMS(DataBase Management System) : 기본적으로 데이터베이스는 많은 사람이 공유할 수 있어야 하고,
            동시 접근이 가능해야 하는 등 많은 요구 사항이 존재함.
                ...
                대부분 개발자들이 편하게 DB라고 이야기 하는 MySQL, Oracle, DBeaver와 같은 것들은
                실제로는 DB가 아니라 DBMS에 해당

            관계형 DBMS
                Relational DBMS을 줄여서 RDNMS라고 부름. 테이블 형태로 이루어진 데이터 저장소.
            ...
            H2, MySQL
                ...
                H2 - 자바로 작성되어 있는 RDBMS : 스프링 부트가 지원하는 인메모리 관계형 데이터베이스
                    데이터를 다른 공간에 따로 보관하는 것이 아니라 애플리케이션 자체 내부의 데이터를 저장하는 특징
            ...
            필수 용어
                1) 테이블 : DB에서 데이터를 구성하기 위한 가장 기본적인 단위 행과 열로 구생되며
                ...
                3) 열(column) : 테이블의 구성 요소 중 하나이며, 행에 저장되는 유형의 데이터.
                    예를 들어 회원 테이블이 있다고 할 때, 열을 각 요소에 대한 속성을 나타내며(Java에서는 field)
                ...
                4) 기본키(Primary Key) : 행을 구분할 수 있는 식별자. 이 값은 테이블에서 유일해야 하며 '중복값을 가질 수 없음'
                    ...
                    또한 기본키의 값은 수정되어서는 안 되며 유효한 값이어야함. 즉, NULL이 될 수 없음
        ...
        2. ORM이란?
            Objective-Relational Mapping이라고 하며 자바의 객체와 데이터베이스를 연결하는 프로그래밍 기법
                예를 들어 데이터베이스에 age, name 컬럼에 28, 홍길동이라는 값이 있다고 가정했을 때, 이것을 자바에서 사용하려면
            ...
            장점 :
                1) SQL을 직접 작성하지 않고 사용하는 언어(여기서를 Java)로 데이터베이스에 접근 가능
        ...
        스프링 데이터 JPA에서 제공하는 메서드 사용
            MemberRepository.java 파일에서 create test해주세요
