...
/*
    1. 사전 지식 : API와 REST API
        ...
        식당으로 알아보는 API
            ...
            의 형태를 띄고 있는데, 여기서는 손님은 클라이언트(client), 주방에서 일하는 요리사를 서버(server)라고 생각하시면 됩니다.
            ...
            웹의 장점을 최대한 활용하는 형태라고 알려진 REST API
                ...
                장단범을 고려했을 때, 주소와 HTTP 메서드만 보고 요청의 내용을 파악하라 수 있다는 장점으로
                REST하게 디자인한 API를 보고 RESTful API라고 부르기도 하는 편.

                REST API를 사용하는 방법
                    규칙 1. URL에는 동사를 쓰지 말고, 자원을 표시한다.
                        *자원 : 가져오는 데이터를 의미. 예를 들어 학생 중에 id가 1인 학생의 정보는 가져오는 URL은
                        1)예시 : /students/1
                    ...
                    동사를 사용해서 생기게 되는 추후의 문제점 예시 -> 데이터를 요청하는 URL을 설계할 때
                        A개발는 get, B개발자는 show를 쓰면 get-student, show-data 등으로 협의가 이루어지지 않은 설계가
                        될 가능성이 있다.
                    ...
                    규칙 2. 동사는 HTTP 메서드로
                        ...
                        예를 들어 블로그에 글을 쓰는 설계를 한다고 가정했을 때
                            ...
                            2) 블로그 글을 '추가'하는 API          : POST/articles   -> 추가가 완료되기 전이라 DB상의 id가 없음
                        ...
                        * GET / POST 등은 URL에 입력하는 값이 아니라 내부적으로 처리하는 방식을 미리 정하는 것으로
                          실제로 HTTP 메서드는 내부에서 서로 다른 함수를 처리하는데, 대놓고 적는 일은 잘 없습니다.
