...
-- 이상의 쿼리의 문제점은 결합 기능을 지닌 UNION의 결과 값이 select * from users;와
-- 동일하다는 점입니다.
...
-- UNION은 중복 제거 기능 / UNION ALL은 중복 포함 출력

-- 참고 UNION ALL 사용시 SELECT에서 컬럼 선별 예시
(SELECT id, phone, city, country		-- 일부 컬럼만 지정해서 출력
	FROM users)
UNION ALL
(SELECT id, phone, city, country		-- 기준이 되는 첫번재 select절에서 선택하는
...
-- 이단 UNION ALL을 통해서 최종 결과 형태 확인 후에 -> UNION을 적용하는 식으로
-- 프로세스가 짜여있는 편

-- users에서 country가 Korea인 회원 정보만 추출하고(1번 추출), 
-- Mexico인 회원 정보만 추출해서(2번 추출) 결합해보자
-- (단, 컬럼은 id, phone, city, country만 출력하고, 최종 결과 집합은 country 기준 알파벳순) ASC
...
-- 두 결과 집합은 UNION ALL을 사용해 하나로 결합하세요(단, 최종 결과는 최신순으로 정렬)
-- 1번 쿼리
...
-- YYYY-MM-DD 형식으로 지정돼있다면, 문자열 비교 결과와 실제 남짜 비교 결과가 동일하게 작용함.
-- 그래서 MM-DD-YYYY 형태로 돼있다면 오류가 발생할 가능성이 있습니다.



-- 2. users에서 USA에 거주 중이면서 마케팅 수신에 동의(1)한 회원 정보와 France에 거주 중이면서
-- 마케팅 수신에 동의하지 않은(0) 회원 정보는 SELECT로 각각 추출하고, 두 결과 집합을 UNION ALL을
...
-- 3. 이건 풀지 마세요. 문제만 적어놓고 같이 풀겠습니다.
--		UNION을 활용하여 orderdetails와 products를 FULL JOIN조건으로 결합하여 출력하세요.
-- 		-> 굳이 이런 형식으로 시험 문제 및 실무에서까지 이용하는 경우는 거의 없습니다.
...
-- productx에서 name(제품명)과 price(정상가격)을 모두 불러오고, '평균 정상 가격을
...
SELECT * FROM users;
-- users에서 city별 회원 수를 카운트하고, 회원 수가 3명 이상인 도시 명과 회원수를 출력해보자.
-- (단, 회원수를 기준으로 내림차순)
...
-- orders와 staff를 활용해 last_name이 Kyle이나 Scott인 직원의 담당 주문을 출력한다면?
-- (단, 서브쿼리 형태를 활용하자)

SELECT * FROM orders, STAFF;

-- 내가 쓴 답안
SELECT o.id, s.last_name
...
-- WHERE 절 내에서 필터링 조건 지정을 위해 중첩딘 서브 쿼리를 작성 가능
-- WHERE 에서 IN 연산자와 함께 서브 쿼리를 활용할 경우 :
-- 	컬럼 개수와 필터링 적용 대상 컬럼의 개수가 '일치'해야만 합니다.

-- 이상의 코드에서 서브쿼리의 결과 도출되는 컬럼의 숫가 = 1 (staff테이블의 id) / 행 = 2
...
-- orders에서 주문 월(order_date 컬럼 활용)이 2015년 7월인 주문정보를,
...
-- 1)과 2)의 INNER JOIN 구문 -> FROM절에서 이루어진다
SELECT *
	FROM(SELECT *
			FROM ORDERS
			WHERE SUBSTR(order_date, 1, 7) = "2015-07") o	-- 1)의 결과가 테이블 이었기 때문에 별칭 o를 사용
		INNER JOIN 
		(SELECT *
			FROM ORDERDETAILS
...
-- FROM에서 사용되는 서브 쿼리 : '인라인 뷰', 마치 테이블처럼 서브 쿼리를 결과값을 사용 가능
-- 또한 FROM에서 2개 이상의 서브 쿼리를 활용하여 JOIN 연산 가능
-- 이 때 조인 연산을 위해 별칭을 생성 가능한데 서브 쿼리가 끝나는 괄호 뒤에 공백을 한 칸 주고
-- 원하는 별칭을 쓰면 된다(order o / orderdetails od와 같은 방식)
...
--	WHERE에 작성하는 필터링 대상 컬럼 개수가 일치 해야함 -> 이 때 필터링 대상 컬럼이 2개 이상이면
--	()로 묶어서 작성해야 합니다.

-- 1. 데이터 그룹화히기(GROUP BY + 집계함수)
...
SELECT country, city, SUM(is_auth) 		-- is_auth가 1이면 본인인증한거니까 is_auth들의 합
