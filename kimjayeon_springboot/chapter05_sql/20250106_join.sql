...
-- 추가 문법이 적용됐음
-- 회원 정보와 주문정보를 하나로 결합하기 위해 users와 orders를 INNER JOIN(추후설명)으로
-- 묶고, '후속조건'으로 '주문정보가 있는 회원의 정보만 출력하기 위해' u.id = o.user_를

-- 여기서 여러분들에게 연습문제시만 설명했던 부분을 명확하게 하고 넘어가야 합니다.
...
-- 그래서 이후에 FROM절에서 다수의 테이블명을 기입하게 될 경우에 별칭을 통해서 정리하여
-- SQL문을 효율적으로 사용할 수 있게 됨.

-- 이상의 문제에서의 조건은 '주문 정보가 있는 회원의 정보만 출력'하는 것이므로, oders 내에
-- users_id가 일부 기준이 되어야 합니다.

-- 왜냐하면 users 내에 있는 id는 1부터 끝까지 있으니까요
-- users 테이블에는 회원 id가 id컬럼에 기록돼있고, oders 테이블에는 회원 id가 users_id로
-- 공통된 부분을 지정하는 컬럼이 존재하므로 둘을 연결시킬 수 있는데, 이때 사용하는 전치사가
-- 'ON'

-- JOIN 적용 후 결과를 보기 좋게 정렬하도록 SQL 문 수정
-- 회원 id를 기준으로 오름차순하는 조건.
-- ORDER BY애서도 테이블 별칭으로 정렬할 컬럼을 지정 가능.
...
-- 테이블에서 개별 행을 유일하게 구분 짓습니다. 따라서 키 값은 컬럼 내에서 중복되지 ㅇ낳으며
-- 개별 행을 구분해야 하므로 null 값을 가질 수 없습니다
-- (nullable = false로 Entity 클래스에서 지정했습니다.)

-- cf) 키 값은 테이블 내에서 고유한 값을 가지므로 한테이블에서 개수를 계산할 때 중복되진 않는다.
...
-- 2. 외래 키(Froeign Key) : 다른 테이블에서 가지는 기존 테이블의 값

-- FK는 다른 테이블의 고유한 키 앖인 PK를 참조한다.(orders에서 o.id가 FK에 해당,
...
-- 2. LEFT JOIN
-- users와 orders를 하나로 결합해 출렵합니다(단, 주문 정보가 회원의 정보도 모두 출력)
...
-- 이상의 예시에서는 users 케이블이 왼쪽에 있으므로, users 테이블의 모든 결과값을 가져오는데,
...
-- 이를 이용해서 SELECT에서도 * 대신에 표시할 컬럼을 지정할 수 있느데,
...
-- 주문 정보가 없는 회원의 주문 정보도 모두 출력하고, 다음 컬럼도 출력하자. u.id, u.username,
-- u.phone, o.user_id, od.order_id, od.product_id). -> JOIN이 두 번 일어납니다.

-- FROM 내에서는 JOIN을 중첩헤서 횟수 제한 없이 사용 가능합니다.
-- 첫 번째 JOIN의 ON절 뒤에, 두 번째 JOIN 절을 작성하면 됩니다.

SELECT u.id, u.username, u.phone, o.user_id, od.order_id, od.product_id
	from users u left join ORDERS O  on u.id = o.USER_ID 	-- 여기까지가 첫 번째 join
...
-- (출력된다는 것을 의미합니다.)
-- RIGHT JOIN의 경우에는 오른쪽 테이블의 위치한 값을 전부 가지고 온다는 것을 의미합니다.
...
-- 키 값은 여러개가 있을수 있어, 어떤 값을 기분으로 할 지 ON에서 명시합니다.
-- 다중 키 값을 설정할 때는 ON에서 각 조건을 AND로 열결합니다.

-- 정리 2. JOIN 중첩
-- FROM [테이블명1] a(별칭) [INNER/LEFT/RIGHT/CROSS JOIN] [테이블명2] b(별칭)
-- ON [JOIN 조건1]
-- [INNER/LEFT/RIGHT/CROSS JOIN] [테이블명2] b(별칭)
-- ON [JOIN 조건2]

-- FROM 내에서 JOIN을 여러 번 중첩 사용 가능. 앞의 JOIN의 ON절 뒤에 새로운 JOIN 명령어를 
...
-- INNER JOIN은 각 테아블의 키 밧이 '일치하는 행만' 가져옵니다. 집합으로 보면 교집합.
-- 가장 기본적인 JOIN문으로, 간혹 INNER를 생략하고 JOIN만 쓰기도 합니다(defualt JOIN 구문)
...
-- OUTER JOIN은 LEFT JOIN, RIGHT OUTER JOIN, FULL OUTER JOIN을 포괄하는 용어.
-- OUTER 생략가능
...
-- 정리 5. CROSS JOIN
-- FROM [테이블명1] a CREOSS JOIN [테이블명2] b
-- CROSS JOIN은 두 테이블을 결합했을 때 각 테이블의 행으로 만들 수 있는 모든 조합을 결과값으로
-- 도축하는 연산 -> 카테시안 곱 이라고 합. -> 쓸 때 주의하세요

-- 주의 사항 : FULL OUTER vs. CROSS JOIN
-- FULL OUTER JOIN의 경우 ON 조건에 부합할 때만 결과 값을 도출하는 반면에,
