...
/*
    1. 스프링 시큐리티의 모든 기능을 사용자지 않게 설정하는 코드
    ...
    3. 특정 경로에 대한 엑세스 설정
        requestMatcher() : 특정 요청과 일치하는 url에 대한 엑세스를 설정
    ...
    5. 로그아웃 설정
        ...
        invalidHttpSession() : 로그아웃 이후 세션을 전체 삭제할지 여부를 설정 -> 저희는 true

    6. csrf 설정을 비활성화 - CSRF 공격을 장비하기 위해서 사용하는건데, 실습 좀 편하게 하려고 비활성화 했습니다
        (정보처리기사필기에서 본적 있음)

    7. 인증 관리자 관련 설정. 사용자 정보를 가져올 서비스를 재정의하거나, 인증방법, 예를 들어
