...
/*
    학습 목표 : 로그인 / 회원 가입 기능을 구현
        ...
        -> defaultSuccessUrl로 설정한 /article로 리다이렉트 하거나 csrf를 disable하거나 하는 등의 작업
        ...
        (2) 스프링 시큐리티
            : 스프링 기반 애플리케이션의 보안을 담당하는 스프링 하위 프레임워크 보안 관련 옵션을 다수
                제공해줌.
                CSRF, 세션 공격을 방어해주고(-> 정처기 등에 자주 나옴), 요청 해더도 보안 처리를 해주므로
        ...
        (3) 필터 기반으로 동작하는 스프링 시큐리티
            ...
            모두 외우야 하는거 x, 다만 로그인 과정이 어떤 흐름으로 이어지는 지를 이해하면 더 잘 활용 가능
            일단은 회원 가입 / 로그인 관련 부분을 코딩으로 풀어나갈 예정

    2. 회원 도메인 만들기
        스프링 시큐리티를 확용한 인증, 인가 기능 구현 예정
        회원 정보를 저장할 테이블을 만들고,
        테이블과 연결할 도메인을 만들고,
