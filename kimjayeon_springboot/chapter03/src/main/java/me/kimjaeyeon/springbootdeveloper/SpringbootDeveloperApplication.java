...
/*
            ...
            2) 비지니스(Business)
                모든 비지니스 로직 처리
                    * 비지니스 로직 : 서비스를 만들기 위한 로직을 의미하며, 웹 사이트에서 벌어지는
                        모든 작업 -> 주문 서비스라고 한다면 주문 개수, 가격 등의 데이터를 처리가히
    ...
    스프링 부트 프로젝트 디렉토리 구성
        : 스프링 부트에는 정해진 구조가 없지만, 추천 프로젝트 구조가 있고, 이를 많은 개발다즐이 따르드
            수업 시에도 해당 구조를 따라 개발할 예정.

            main : 실제 코드를 작성하는 공간, 프로젝트의 실행에 필요한 소스 코드나 리소스 파일은
                모두 이 폴더 안에 들어 있음(프로젝트 생성시 자동 구현됨)

            test : 프로젝트는 소스 코드를 테스트할 목적의 코드나 리소스 파일이 들어있음
            ...
            main 디렉토리 구성하기
                ...
                03 단계 -> 스프링 부트 설정을 할 수 있는 application.yml 파일을 생성
                    -> 해당 파일은 스프링 쿠트 서버가 실행되면 자동으로 로딩되는 파일로,
                    데이터베이스의 설정 정보, 로깅 설정 정보 등이 들어갈 수도 있고, 직접 정의할 때
                    사용하기도 함. 앞으로 프로제그를 진행하며 자주 사용할 파일입니다.
                    resources 우클릭 -> new file -> application.yml

    2. 스프링 부트 3 프로젝트 발전시키기

        각 계층의 코드를 추가할 예정 -> 이를 통해 계층이 무엇이고, 스프링 부트에서는 계층을 어떻게
            나누는지를 감을 잡아나갈실 수 있도록 수업하겠습니다(목요일에 이루어질 예정)
        지금은 의존성을 추가한 다음에 -> 프레젠테이션 계층, 비지니스 계층, 퍼시스턴스 계층 순서를
            코드를 추가할 예정입니다.
        ...
        implementation : 프로젝트 코드가 컴파일 시점과 런타임에 모두 해당하는 라이브러리를 필요로 할때
