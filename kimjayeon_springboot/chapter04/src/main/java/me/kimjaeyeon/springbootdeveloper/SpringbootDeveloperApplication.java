...
/*
    테스트코드 개념 익히기
        ...
        작성하는 코드
        보통 테스트 코드 관련 공부는 본 개발 공부를 하느라 미루는 경우가 많은데 좀익숙해지면
        솔직히 이거 완전 좋습니다
        ...
        테스트 코드란?
            테스트 코드는 test 디렉토리에서 작업함 -> 프로젝트 생성시 src의 하위 폴더,
                main과 같은 라인에 test폴더가 있습니다
            테스트 코드는 다양한 패턴이 있는데 일단 저는 given-when-then 패턴을 사용할 예정
        ...
        이상의 코드를 확인하면 세 부분으로 나위어져 있음. 메뉴를 저장하기 위해 준비하는 과정인 given절
        실제로 메뉴를 저장하는 when절
        메뉴가 잘 추가되었는지 확인하는 then절

        2. 스프링 부트 3와 테스트
            스프링 부트는 애플리케이션 테스트하기 위한 도구와 애너테이션을 제공
            sping-boot-starter-test 스타터에 테스트를 위한 도구가 모여있음

            스프링 부트 스타터 테스트 목록(중 많이 쓰이는 것 두 개만 소개)
                ...
                JUnit이란?
                    ...
                    특징 :
                        ...
                        5) 자동실행, 자체 결과를 확인하고 실패한 지점을 보여줌

           test 폴더에 JUnitTest.java 파일 생성
           ...
           AssertJ로 검증문 가독성 높이기
                ...
                Assertions.assertEquals(sum, a + b);를 보면 기대값과 비교값이 잘 구분되지 않습니다
                ...
                9) isNegative : 음수인지 검증
                10) isPositive : 양수인지 검증
